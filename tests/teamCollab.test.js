...
await test('happy path: create team, invite, accept', async t => {
  await resetStore();
  const teamRes = await createTeam({ requesterId: 'u1', name: 'Alpha', description: 'First team' });
  assert.equal(teamRes.status, 201);
  const teamId = teamRes.team.id;

  const inviteRes = await inviteMember({ requesterId: 'u1', teamId, email: 'bob@example.com' });
  assert.equal(inviteRes.status, 202);
  const token = inviteRes.invitationId;

  const acceptRes = await respondInvitation({ token, responderId: 'u2', decision: 'accept' });
...
await test('edge case: duplicate invitation tokens are unique', async t => {
  ...
  const token1 = (await inviteMember({ requesterId: 'u1', teamId: team.id, email: 'bob@example.com' })).invitationId;
  const token2 = (await inviteMember({ requesterId: 'u1', teamId: team.id, email: 'bob@example.com' })).invitationId;
...
await test('edge case: invalid token on response', async t => {
  await resetStore();
  await assert.rejects(
    respondInvitation({ token: 'invalid-token', responderId: 'u2', decision: 'accept' }),
    /Invalid token/
  );
});
...
await test('edge case: owner cannot remove themselves', async t => {
  await resetStore();
  const teamId = (await createTeam({ requesterId: 'u1', name: 'Delta' })).team.id;
  await assert.rejects(
...
await test('edge case: audit log pagination and date filter', async t => {
  ...
  const limited = (await getAuditLog({ requesterId: 'u1', teamId, limit: 2 })).logs;
  assert.equal(limited.length, 2);
  const future = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
  const empty = (await getAuditLog({ requesterId: 'u1', teamId, startDate: future })).logs;
...
await test('edge case: permission updates only by owner', async t => {
  ...
  const user = await createUser({ name: 'Owner', email: 'owner@example.com', password: 'pass123' });
  const ownerId = user.id;
  const teamId = (await createTeam({ requesterId: ownerId, name: 'Zeta' })).team.id;
  const ok = await updatePermissions({ requesterId: ownerId, teamId, permissionMatrix: { canInvite: true } });
  assert.equal(ok.status, 200);
  assert.deepStrictEqual(ok.permissions, { canInvite: true });
  // another user attempting update
  const otherUser = await createUser({ name: 'Other', email: 'other@example.com', password: 'pass123' });
  await assert.rejects(
    updatePermissions({ requesterId: otherUser.id, teamId, permissionMatrix: { canInvite: false } }),
